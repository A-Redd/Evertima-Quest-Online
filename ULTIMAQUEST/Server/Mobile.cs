...
namespace Server
{
	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable, IDamageable
	{
        ...
        private int m_MobStr, m_MobCon, m_MobAgi, m_MobDex, m_MobWis, m_MobInt, m_MobCha;
        private int m_MobLevel;
        private int m_ExpRewarded;
        private int m_MobCurXP;
        private int m_ATK;
        private int m_APR;
        private int m_ATKSpeed;
        private int m_Fame, m_Karma;
		private AccessLevel m_AccessLevel;
        private MobClass m_MobClass;
		...
		private static int m_MaxPlayerResistance = 60000;
		...
		*/
		...
		private class ManaTimer : Timer
		{
			...
			protected override void OnTick()
			{
				...
				{
					m_Owner.Mana += 1 + (int)(m_Owner.Skills[SkillName.Meditation].Value / 10);
                    m_Owner.Mana += (int)(m_Owner.Skills[SkillName.Focus].Value / 10);
                    m_Owner.Mana += m_Owner.ShortTermMurders;  
                }
		...
		public virtual void Deserialize(GenericReader reader)
		{
			...
			switch (version)
			{
                case 38:
                {
                        m_MobStr = reader.ReadInt();
                        m_MobCon = reader.ReadInt();
                        m_MobAgi = reader.ReadInt();
                        m_MobDex = reader.ReadInt();
                        m_MobWis = reader.ReadInt();
                        m_MobInt = reader.ReadInt();
                        m_MobLevel = reader.ReadInt();
                        m_ExpRewarded = reader.ReadInt();
                        m_MobCurXP = reader.ReadInt();
                        m_ATK = reader.ReadInt();
                        m_APR = reader.ReadInt();
                        m_ATKSpeed = reader.ReadInt();
				...
				case 0:
					{
                        ...
                        m_MobClass = (MobClass)reader.ReadByte();
		...
		public virtual void Serialize(GenericWriter writer)
		{
            ...
            //
            writer.Write(m_MobStr);
            writer.Write(m_MobCon);
            writer.Write(m_MobAgi);
            writer.Write(m_MobDex);
            writer.Write(m_MobWis);
            writer.Write(m_MobInt); 
            writer.Write(m_MobCha);
            writer.Write(m_MobLevel);
            writer.Write(m_ExpRewarded);
            writer.Write(m_MobCurXP);
            writer.Write(m_ATK);
            writer.Write(m_APR);
            writer.Write(m_ATKSpeed);
        ...
        private static readonly string[] m_MobClassNames = new[]
        {
            "Warrior", "Cleric", "Paladin", "Ranger", "ShadowKnight", "Druid", "Monk", "Bard", "Rogue",
            "Shaman", "Necro","Wizard","Mage","Enchanter","BeastLord","Berzerker"
        };

        public static string GetMobClassName(MobClass mobclass)
        {
            return m_MobClassNames[(int)mobclass];
        }
        ...
        public int MobLevel
        {
            get { return m_MobLevel; }
            set { m_MobLevel = value; }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public int ExpRewarded
        {
            get { return m_ExpRewarded; }
            set { m_ExpRewarded = value; }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public int MobCurXP
        {
            get { return m_MobCurXP; }
            set { m_MobCurXP = value; }
        }
        ...
        {
            get { return m_MobClass; }
            set
            {
                MobClass oldValue = m_MobClass;
                ...
                if (oldValue != value)
                {
                    ...
                    SendMessage("Your CLass has been changed. You are now a {0}.", GetMobClassName(value));

                    OnMobClassChanged(oldValue);
                }
            }
        }
        ...
        public virtual void OnMobClassChanged(MobClass oldLevel)
        { }
